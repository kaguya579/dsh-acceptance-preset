...
/** 引用分类（单一入口纯函数）：五种分桶，主循环按桶分发。 */
export function classifyRef(ref) {
  if (ref.kind === 'include') return ref.local === true ? 'local_include' : 'system_include'
  if (ref.kind === 'import' && ref.ref.endsWith('.*')) return 'wildcard'
  if (ref.kind === 'import' && ref.ref.startsWith('static ')) return 'static'
  if (ref.ref.startsWith('./') || ref.ref.startsWith('../')) return 'relative'
  return 'external'
}
...
export function buildDependencyGraph(codeFiles, perFileImports) {
  ...
  for (const item of perFileImports) {
    ...
    for (const ref of imports) {
      const bucket = classifyRef(ref)
      if (bucket === 'local_include') {
