...
//   data  —— 仅对需要解析的文件保留字节（code ≤ 2MB、doc ≤ 20MB、image ≤ 8MB），其余为 null
...
export async function scanDeliverable(adapter, deliverablePath) {
  const stat = await adapter.stat(deliverablePath)
  if (stat === null) throw new Error(`交付物不存在：${deliverablePath}`)
  if (stat.type === 'directory') return scanDirectory(adapter, deliverablePath)
  if (stat.type === 'file') {
    const lower = deliverablePath.toLowerCase()
    if (lower.endsWith('.zip')) return scanZip(adapter, deliverablePath)
    if (lower.endsWith('.tar.gz') || lower.endsWith('.tgz')) return scanTarGz(adapter, deliverablePath)
...
/** 目录扫描：递归列出文件，跳过排除目录。 */
async function scanDirectory(adapter, root) {
  const entries = []
  const errors = []
...
async function scanZip(adapter, zipPath) {
  ...
  const unzip = new Unzip((file) => {
    ...
    if (rel === null || excludedByDir(rel)) {
      file.terminate() // 非法路径/排除目录：静默跳过
      return
    }
    const chunks = []
    let received = 0
...
/** tar.gz 解包扫描（同上约束）。 */
async function scanTarGz(adapter, tarPath) {
