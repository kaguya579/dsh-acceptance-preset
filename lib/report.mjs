...
export const DASHBOARD_FILENAME = '验收仪表盘.html'
...
export function renderDashboard(facts) {
  ...
  sections.push(`<section><h2>概览</h2>${table(
    ...
    [
      ...
      ['依赖边', deps.edges?.length ?? 0],
      ['模块', deps.modules?.length ?? 0],
      ['依赖环', deps.cycles?.length ?? 0],
      ['真未解析', deps.unresolved?.length ?? 0],
      ['外部引用', (deps.external ?? []).reduce((sum, entry) => sum + entry.count, 0)],
      ['超阈值函数', metrics.over_threshold?.functions?.length ?? 0],
      ['重复片段', arch.duplicates?.fragments?.length ?? 0],
    ],
  )}</section>`)
