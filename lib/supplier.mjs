...
import { createRoundStore, snapshotFromRecord, computeChanges } from './rounds.mjs'
...
export async function buildSupplierLedger({ adapter, outRoot, supplier, projects }) {
  ...
  for (const project of projects.slice(0, PROJECTS_CAP)) {
    let roundDirs = []
    try {
      roundDirs = await store.listRounds(project)
    } catch {
    ...
    for (const roundDir of roundDirs.slice(0, ROUNDS_CAP)) {
      const summary = await summarizeRound(store, roundDir, previousRecord)
...
async function summarizeRound(store, roundDir, previousRecord) {
  const row = {
  ...
  let record = null
  try {
    record = await store.readRecord(roundDir.project, roundDir.round)
  } catch {
    // 记录缺失：行保留空值
  }
  ...
  try {
    const facts = await store.readFacts(roundDir.project, roundDir.round)
