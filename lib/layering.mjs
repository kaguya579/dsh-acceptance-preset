...
/** 模块路径匹配：`/**` 后缀=前缀匹配，否则精确匹配（大小写不敏感）。内部使用。 */
function matchModulePattern(module, pattern) {
...
/** 模块 → 层名（按 layers 声明顺序首个命中）；未命中返回 null。内部使用。 */
function layerOfModule(module, layers) {
  for (const layer of layers) {
...
export function validateLayering(rules, moduleEdges) {
  ...
  for (const edge of moduleEdges ?? []) {
    ...
    if (!allowed && violations.length < VIOLATIONS_CAP) {
      violations.push({
        rule: `${fromLayer}→${toLayer}`, // 缺失的允许依赖对（白名单未声明）
        from: edge.from,
        to: edge.to,
