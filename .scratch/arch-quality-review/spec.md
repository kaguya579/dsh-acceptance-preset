# Spec: arch-quality-review（架构与质量评审第一刀）

Status: ready-for-agent

2026-08-16 `/grill-with-docs` 会话收敛后、`/to-spec` 产出。本 spec 覆盖第一刀：确定性层「架构事实」四件套 + 事实包 schema 升版 + agent 语义层「架构与质量评审」节。领域术语见 `CONTEXT.md`；决策依据见 ADR-0004（架构事实扩展）、ADR-0005（优化项咨询性定位）。
